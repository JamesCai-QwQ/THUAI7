...
bool Chase(IShipAPI& api,int last_seen_gridx,int last_seen_gridy);  //追击，使用时必须加前置语句once_met_enemys=false;之所以未加到函数里面是因为怕与attackship/goplace循环调用
...
bool attack(IShipAPI& api)  // 正在改，缺陷还很多
{
    ...
    {
        if (HP < 1500 && totalenenmyhp > HP && map[cellx][celly]!=THUAI7::PlaceType::Shadow)  // 条件可能还要改，丝血隐蔽
        {
            hide(api);
            api.Wait();
        }
